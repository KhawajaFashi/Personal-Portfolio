    ...
    <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-24">
      ...
      <div className="lg:col-span-7 flex flex-col gap-6">
        <div className="section-label">
          <span className="w-1.5 h-1.5 bg-accent-gold animate-pulse" />
          overview
        </div>
        ...
        <div className="flex flex-wrap gap-4 pt-2">
          <a href="#projects" className="editorial-btn-primary focus:outline-none focus:ring-2 focus:ring-accent-gold/40">
            View Selected Works
          </a>
          <a href="#contact" className="editorial-btn-secondary focus:outline-none focus:ring-2 focus:ring-accent-gold/40">
            Get In Touch
          </a>
        </div>
      </div>

      {/* ── Right Column (40% width) — Query Latency Dashboard widget ── */}
      <div className="lg:col-span-5 w-full">
        <div className="flex flex-col gap-6 font-mono text-xs bg-obsidian-card border border-obsidian-border border-t-4 border-t-accent-gold shadow-[0_1px_3px_rgba(0,0,0,0.5)] p-5">
          {/* Header */}
          ...
          <div className="grid grid-cols-2 bg-obsidian-bg p-1 border border-obsidian-border text-[10px]">
            <button
              type="button"
              onClick={() => {
                if (!isSimulatingQueries) {
                  setPromiseMode("sequential");
                  setQueryProgresses(new Array(17).fill(0));
                  setLatencyReadout(850);
                }
              }}
              disabled={isSimulatingQueries}
              className={`py-2 rounded-sm transition-all cursor-pointer font-bold outline-none focus-visible:ring-1 focus-visible:ring-crimson-alert/50 disabled:opacity-40 disabled:cursor-not-allowed ${
                promiseMode === "sequential"
                  ? "bg-crimson-alert text-primary-text"
            ...
            <button
              ...
              onClick={() => {
                if (!isSimulatingQueries) {
                  setPromiseMode("parallel");
                  setQueryProgresses(new Array(17).fill(0));
                  setLatencyReadout(230);
                }
              }}
              disabled={isSimulatingQueries}
              className={`py-2 rounded-sm transition-all cursor-pointer font-bold outline-none focus-visible:ring-1 focus-visible:ring-accent-gold/50 disabled:opacity-40 disabled:cursor-not-allowed ${
                promiseMode === "parallel"
                  ? "bg-accent-gold text-obsidian-bg"
                  : "text-muted-text hover:text-primary-text"
              }`}
            >
              Promise.all (230ms)
          ...
          <div className="flex flex-col gap-1.5 bg-[#090909] p-4 border border-obsidian-border rounded-sm">
            ...
            <div
              className="grid gap-1 h-20 items-end"
                  ...
                  <div className="w-full bg-[#181818] border border-obsidian-border/80 rounded-sm relative overflow-hidden h-full flex flex-col justify-end">
                    <div
                      className={`w-full transition-all duration-75 ${
                        promiseMode === "parallel" ? "bg-accent-gold" : "bg-crimson-alert"
                      }`}
                      style={{ height: `${prog}%` }}
                    />
                  </div>
          ...
          <button
            ...
            disabled={isSimulatingQueries}
            className={`w-full font-mono py-2 text-center rounded-sm border transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed outline-none focus-visible:ring-2 ${
              promiseMode === "parallel"
                ? "bg-accent-gold/10 hover:bg-accent-gold/20 border-accent-gold/40 hover:border-accent-gold text-accent-gold focus-visible:ring-accent-gold/30"
                : "bg-crimson-alert/10 hover:bg-crimson-alert/20 border-crimson-alert/40 hover:border-crimson-alert text-crimson-alert focus-visible:ring-crimson-alert/30"
            }`}
          >
